...
public class CursedBladeEffect {
    public static ItemEffect itemEffect = ItemEffect.get("goety_revelation.cursed_blade");

    @OnlyIn(Dist.CLIENT)
    public static void init() {
        try {
            StatGetterEffectLevel statGetterEffectLevel = new StatGetterEffectLevel(itemEffect, 1.0D);
            GuiStatBar effectBar = new GuiStatBar(0, 0, 59, "goety_revelation.effect.cursed_blade.name", 0.0D, 100.0, false, statGetterEffectLevel, LabelGetterBasic.decimalLabel, new TooltipGetterCursedBlade());
        ...
        if (event.getSource().getEntity() instanceof Player attacker) {
            ...
            if (itemStack.getItem() instanceof ModularItem modularItem) {
                ...
                if (Math.random() * 100D <= level) {
                    ...
                    event.getEntity().addEffect(new MobEffectInstance(GoetyEffects.CURSED.get(), time, 0));
                }
            }
        }
    }
    ...
    static class TooltipGetterCursedBlade implements ITooltipGetter {
        ...
        public String getTooltipBase(Player player, ItemStack itemStack) {
            ...
            return I18n.get("goety_revelation.effect.cursed_blade.tooltip", level, efficiency);
        }
        ...
        public String getTooltipExtension(Player player, ItemStack itemStack) {
            return I18n.get("goety_revelation.effect.cursed_blade.tooltip_extended");
        }
    }
}
