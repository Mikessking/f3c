...
public class PositionBladeEffect {
    ...
    public static void init() {
        try {
            ...
            GuiStatBar effectBar = new GuiStatBar(0, 0, 59, "goety_revelation.effect.position_blade.name", 0.0D, 100.0, false, statGetterEffectLevel, LabelGetterBasic.decimalLabel, new TooltipGetterCursedBlade());
        ...
        if (event.getSource().getEntity() instanceof Player attacker) {
            ...
            if (itemStack.getItem() instanceof ModularItem modularItem) {
                ...
                if (Math.random() * 100D <= level) {
                    ...
                    int effectLevel = (int) (modularItem.getEffectEfficiency(itemStack, itemEffect));
                    event.getEntity().addEffect(new MobEffectInstance(GoetyEffects.ACID_VENOM.get(), time, Math.max(0, effectLevel - 1)));
                }
            }
        }
    }
    ...
    static class TooltipGetterCursedBlade implements ITooltipGetter {
        ...
        public String getTooltipBase(Player player, ItemStack itemStack) {
            ...
            String efficiency2 = String.format("%s", (int) (efficiencyGetter.getValue(player, itemStack) * 3F));
            return I18n.get("goety_revelation.effect.position_blade.tooltip", level, efficiency, efficiency2);
        }
    }
}
